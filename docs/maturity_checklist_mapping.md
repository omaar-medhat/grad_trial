...
- **Stress detection** serves the **real WESAD model** from the self-contained
  package (`backend/models/wesad_vscode_model_package/`, best of a 15-model
  bake-off with a group/leave-subjects split, **DeepDNN** acc 0.93 /
  ROC-AUC 0.98) via `POST /api/ml/predict/stress`, returning
  label/confidence/probabilities/model_name. It's a binary
  `non_stress`/`stress` model over 252 wrist+chest features (BVP/EDA/ECG/EMG/ACC,
  60 s windows), so it runs on **WESAD-format input**, not the bracelet's
  HR/SpO₂/temp/activity. The dashboard's live stress chip uses the deterministic
  `stress_level` heuristic. (Runtime note: the saved preprocessor requires
  **scikit-learn 1.6.1**, pinned in `backend/requirements.txt`.)
